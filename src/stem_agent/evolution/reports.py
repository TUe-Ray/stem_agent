    ...
    def write_report(
        ...
        *,
        ...
        content = [
            ...
            "|---|---:|---:|",
            f"| Promotion score | {baseline.promotion_score:.4f} | {final.promotion_score:.4f} |",
            f"| Task quality | {baseline.task_quality:.4f} | {final.task_quality:.4f} |",
            f"| Train score | {baseline.train_score:.4f} | {final.train_score:.4f} |",
            f"| Validation score | {float(baseline.validation_score or 0.0):.4f} | {float(final.validation_score or 0.0):.4f} |",
            f"| Cost efficiency | {baseline.cost_efficiency:.4f} | {final.cost_efficiency:.4f} |",
            f"| Safety score | {baseline.safety_score:.4f} | {final.safety_score:.4f} |",
            f"| Stability score | {baseline.stability_score:.4f} | {final.stability_score:.4f} |",
            f"| Complexity penalty | {baseline.complexity_penalty:.4f} | {final.complexity_penalty:.4f} |",
            f"| Cost estimate | {baseline.cost_estimate:.4f} | {final.cost_estimate:.4f} |",
            "",
            "## Organism Shape",
            "",
            "### Initial organism",
