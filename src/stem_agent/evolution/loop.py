...
class EvolutionLoop:
    ...
    def _swebench_baseline_genome(self, scenario_name: str) -> Genome:
        ...
        - Pre-step reads test failure output (artifacts/test_failures.txt) so
          locator and patcher know exactly which tests fail and why.
        - Locator instructions reference test failures as primary signal.
        - Revise patcher uses test failures to guide corrections.
        """
        return Genome.model_validate(
            {
                "genome_version": 0,
                "name": "swebench_code_patch_seed_v3",
                "scenario_name": scenario_name,
                "task_diagnosis": {},
                "roles": [
                    ...
                    {"name": "patcher", "description": "Generates unified diff patch directly with write_patch, self-verifies with apply_patch_dry_run, outputs ONLY the diff.", "instructions": "You are fixing a bug. Do ALL of this in ONE turn using tools.\n\n=== DEFINITION OF DONE ===\nProduction-ready means VERIFIED working — your changes must be provably correct.\nYou have tools (read_file, write_patch, apply_patch_dry_run, search_code, inspect_workspace)\nto get executable feedback. Do NOT rely on assumptions or guesses. Prove correctness through\nverification, iterate until verification passes.\n\nYour patch is DONE only when apply_patch_dry_run succeeds CLEANLY.\n=== END DEFINITION OF DONE ===\n\n*** DIRECT PATCH APPROACH: WRITE UNIFIED DIFF, THEN VERIFY ***\nYou will generate the unified diff yourself (you are a capable model),\nwrite it with write_patch, and verify it with apply_patch_dry_run.\nThis avoids file truncation issues that happen with write_file on large files.\n\nWORKFLOW:\n1. read_file('artifacts/test_failures.txt') — test errors\n2. read_file('artifacts/test_file_hints.txt') — test file paths + oracle hints\n3. read_file on the SOURCE files (NOT test files) to see exact code and line numbers\n   - Use offset+limit to zoom in on the relevant function/section\n   - Do NOT try to read the ENTIRE large file — just the relevant lines\n4. Read the problem_statement\n5. *** Write the unified diff directly using write_patch ***\n   - write_patch with your patch_text parameter set to your unified diff\n   - Unified diff format:\n     --- a/path/to/file.py\n     +++ b/path/to/file.py\n     @@ -line_start,count +line_start,count @@\n      context line\n     -old line\n     +new line\n      context line\n   - Most SWE-bench fixes are 1-5 line changes — keep it minimal\n   - Copy context lines EXACTLY from the files you read — do not alter them\n6. *** Verify the patch applies cleanly ***\n   - apply_patch_dry_run with your patch_text parameter\n   - If it fails, the problem is in your line numbers or context — re-read the file and fix\n\nCRITICAL RULES:\n- Fix SOURCE code, never test code.\n- Change as few lines as possible (ideally 1-5 lines).\n- Preserve existing code style and indentation exactly.\n- Copy context lines verbatim from read_file output — do not modify them.\n- Use EXACT line numbers from files you actually read.\n\nCRITICAL: Your FINAL output must be ONLY the unified diff text.\nNo analysis. No markdown fences. No explanation.\nStart with '--- a/' or 'diff --git a/'.", "allowed_tools": ["call_model", "read_file", "write_patch", "apply_patch_dry_run", "search_code", "inspect_workspace"], "temperature": 0.2},
                ...
                "workflow": [
                    {"id": "locate", "role": "locator", "action": "Read test_failures.txt, test_file_hints.txt, AND the problem_statement. Search ONLY source/production directories (NOT test dirs). Find the buggy production code. Read the source files. Report source file paths, line numbers, code snippets, and your fix hypothesis.", "input_from": [], "output_key": "code_locations"},
                    {"id": "self_verifying_patch", "role": "patcher", "action": "Read test_failures.txt AND the TEST FILES (from test_file_hints.txt). Read the locator's source report and the problem_statement. Study what the tests expect vs what the code does. Generate the unified diff directly with write_patch, self-verify with apply_patch_dry_run. Output ONLY the final corrected unified diff.", "input_from": ["code_locations"], "output_key": "final_output"},
                    {"id": "verify_final", "role": "verifier", "action": "Run apply_patch_dry_run on the final patch. Report the result.", "input_from": ["final_output"], "output_key": "verification_result"},
                ],
                "tools": {"builtin": ["call_model", "search_code", "read_file", "write_file", "write_patch", "git_diff", "apply_patch_dry_run", "inspect_workspace"], "generated": []},
                "memory": {},
                "quality_gates": [],
                "self_evaluation": {"rubric": ""},
        ...
        return Genome.model_validate(
            {
                ...
                "task_diagnosis": {},
                "roles": [
                    {"name": "solver", "description": "Single agent that reads, searches, modifies files, generates git diff, patches, and verifies — all in one turn.", "instructions": "You are fixing a bug in a Python codebase. You have access to the full source code in the workspace.\n\n*** NEW APPROACH: MODIFY FILES DIRECTLY, THEN USE GIT DIFF ***\nInstead of writing unified diff format yourself (which small models often get wrong),\nyou will modify the actual source files with write_file, then call git_diff to generate\na 100% accurate patch. Git never hallucinates line numbers or context lines.\n\nWORKFLOW (do ALL of this in one response using tools):\n1. read_file('artifacts/test_failures.txt') — see failing tests and errors\n2. read_file('artifacts/test_file_hints.txt') — see which files are involved\n3. search_code to find the buggy code — focus on the hinted directories\n4. read_file to get the FULL content of the buggy source file\n5. Read the problem_statement from the case input\n6. *** Apply the fix by modifying source files with write_file ***\n   - Copy the file content from read_file, make your change, write the ENTIRE file\n   - write_file({'path': 'path/to/buggy.py', 'text': '<ENTIRE FILE CONTENT WITH YOUR FIX>'})\n7. *** Generate the patch with git_diff() ***\n   - Call git_diff({}) — this returns the EXACT unified diff (100% accurate)\n8. *** Write the diff to artifacts ***\n   - write_patch({'patch_text': '<diff from git_diff>'})\n9. *** Verify the patch applies cleanly ***\n   - apply_patch_dry_run({'patch_text': '<diff from git_diff>'})\n10. If verification fails: the issue is in how you modified the file — re-read and fix\n11. Output ONLY the final raw unified diff — no markdown fences, no explanation\n\nRULES:\n- Change as few lines as possible (ideally 1-5)\n- Preserve existing code style and indentation EXACTLY\n- Write the ENTIRE file with write_file — not snippets\n\n=== PATCH FORMAT EXAMPLES ===\n\nExample 1 — fixing a missing import:\n--- a/module/file.py\n+++ b/module/file.py\n@@ -1,5 +1,6 @@\n import os\n+from collections import OrderedDict\n import sys\n\nExample 2 — fixing wrong comparison:\n--- a/src/utils.py\n+++ b/src/utils.py\n@@ -15,7 +15,7 @@\n def validate(value):\n-    if value > 0:\n+    if value >= 0:\n         return True\n\n=== END EXAMPLES ===\n\nOutput ONLY the raw unified diff. Start with '--- a/filepath' or 'diff --git a/filepath'.", "allowed_tools": ["call_model", "read_file", "write_file", "search_code", "write_patch", "git_diff", "apply_patch_dry_run", "inspect_workspace"], "temperature": 0.2},
                ],
                "workflow": [
                    {"id": "solve", "role": "solver", "action": "Read test failures and hints, search the codebase, read source files, understand the bug, MODIFY source files with write_file, generate EXACT patch with git_diff, verify it, and output ONLY the final corrected unified diff.", "input_from": [], "output_key": "final_output"},
                ],
                "tools": {"builtin": ["call_model", "search_code", "read_file", "write_file", "write_patch", "git_diff", "apply_patch_dry_run", "inspect_workspace"], "generated": []},
                "memory": {},
                "quality_gates": [],
