        ...
        return Genome.model_validate(
            {
                ...
                "roles": [
                    ...
                    {"name": "patcher", "description": "Analyzes test expectations, then produces and self-verifies a patch.", "instructions": "You are fixing a real bug. Follow these steps EXACTLY in order.\n\n=== STEP 1: Gather information ===\n- read_file('artifacts/test_failures.txt') — test error messages\n- read_file('artifacts/test_file_hints.txt') — test file paths\n- read_file on each test file listed — STUDY the test code carefully\n- Read the source files the locator found\n- Read the problem_statement\n\n=== STEP 2: WRITE YOUR ANALYSIS (output as text, not a tool call) ===\nWrite down EXACTLY:\n```\nTEST EXPECTATIONS:\n- Test [name]: expects [specific behavior]. Asserts [specific assertion].\n\nSOURCE CODE BEHAVIOR:\n- File [path], line [N]: [what the code currently does]\n\nROOT CAUSE:\n- The bug is [explanation]. To fix it, need to [specific change].\n\nPLANNED PATCH:\n- File: [path]\n- Change line [N]: [old] → [new]\n- This fixes the bug because [reason]\n```\n\n=== STEP 3: Produce and verify the patch ===\n- write_patch with your planned changes\n- apply_patch_dry_run to verify\n- If it fails: fix line numbers/context, re-verify (up to 2 retries)\n- Output ONLY the final raw unified diff\n\n=== OUTPUT ===\nOutput ONLY the raw unified diff. No analysis, no markdown fences.\n--- a/path/to/source.py\n+++ b/path/to/source.py\n@@ -line,count +line,count @@\n context line\n-old line\n+new line\n", "allowed_tools": ["call_model", "read_file", "write_patch", "apply_patch_dry_run"], "temperature": 0.2},
