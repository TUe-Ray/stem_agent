        ...
        return Genome.model_validate(
            {
                ...
                "roles": [
                    ...
                    {"name": "patcher", "description": "Reads test code + source files, modifies files with write_file, generates patch with git_diff, self-verifies, outputs ONLY the diff.", "instructions": "You are fixing a bug. Do ALL of this in ONE turn using tools.\n\n=== DEFINITION OF DONE ===\nProduction-ready means VERIFIED working \u2014 your changes must be provably correct.\nYou have tools (read_file, write_file, git_diff, apply_patch_dry_run, search_code, inspect_workspace)\nto get executable feedback. Do NOT rely on assumptions or guesses. Prove correctness through\nverification, iterate until verification passes.\n\nYour patch is DONE only when git_diff returns a clean diff AND apply_patch_dry_run succeeds CLEANLY.\n=== END DEFINITION OF DONE ===\n\n*** NEW APPROACH: MODIFY FILES DIRECTLY, THEN USE GIT DIFF ***\nInstead of writing unified diff format yourself (which small models often get wrong),\nyou will modify the actual source files with write_file, then call git_diff to generate\na 100% accurate patch. Git never hallucinates line numbers or context lines.\n\nWORKFLOW:\n1. read_file('artifacts/test_failures.txt') — test errors\n2. read_file('artifacts/test_file_hints.txt') — test file paths\n3. read_file on the test files — study what they expect\n4. *** INDEPENDENTLY read the source files yourself *** — do NOT trust the locator's line numbers.\n   Open the source files with read_file and verify the exact code.\n5. Read the problem_statement\n6. *** Apply the fix by modifying source files with write_file ***\n   - Use read_file to get the file content, make your changes, then write_file with the FULL modified content\n   - write_file({'path': 'sympy/integrals/somefile.py', 'text': '<ENTIRE FILE CONTENT WITH YOUR FIX>'})\n   - Most SWE-bench fixes are 1-5 line changes — modify ONLY what's needed\n7. *** Generate the patch with git_diff() ***\n   - Call git_diff({}) — no arguments needed\n   - This returns the EXACT unified diff that git sees (100% accurate, no hallucination)\n8. *** Write the generated diff to artifacts/predicted.patch ***\n   - Take the \"diff\" field from git_diff output and write it as the final patch\n   - Use write_patch({'patch_text': '<diff from git_diff>'})\n9. *** Verify the patch applies cleanly ***\n   - apply_patch_dry_run({'patch_text': '<diff from git_diff>'})\n   - If it fails, the issue is in how you modified the file — re-read + re-apply\n\nCRITICAL RULES:\n- Fix SOURCE code, never test code.\n- Change as few lines as possible (ideally 1-5 lines).\n- Preserve existing code style and indentation exactly.\n- Write the ENTIRE file with write_file — not just a snippet.\n\nCRITICAL: Your FINAL output must be ONLY the unified diff text.\nNo analysis. No markdown fences. No explanation.\nStart with '--- a/' or 'diff --git a/'.", "allowed_tools": ["call_model", "read_file", "write_file", "write_patch", "git_diff", "apply_patch_dry_run", "search_code", "inspect_workspace"], "temperature": 0.2},
                ...
                "workflow": [
                    ...
                    {"id": "self_verifying_patch", "role": "patcher", "action": "Read test_failures.txt AND the TEST FILES (from test_file_hints.txt). Read the locator's source report and the problem_statement. Study what the tests expect vs what the code does. MODIFY source files directly with write_file, generate the EXACT patch with git_diff, self-verify with apply_patch_dry_run. Output ONLY the final corrected unified diff.", "input_from": ["code_locations"], "output_key": "final_output"},
