        ...
        return Genome.model_validate(
            {
                ...
                "task_diagnosis": {},
                "roles": [
                    {"name": "locator", "description": "Searches source tree for relevant code locations using test failures as guide.", "instructions": "You have access to the full repo source code in the workspace.\n\nFIRST: read_file('artifacts/test_failures.txt') to see which tests fail and what the error messages are.\nSECOND: read_file('artifacts/test_file_hints.txt') to see which test files were modified and which source modules are likely involved.\n\nTHEN: use search_code to find the files, functions, and classes mentioned in the test failures and error tracebacks. Focus your search on the likely source directories from the hints file. Use read_file to read the exact file contents you need to understand the bug.\n\nReport: (1) the failing test names and what they expect, (2) exact file paths and line numbers where the bug is, (3) the key code snippets showing the bug, (4) your hypothesis for what needs to change and why.", "allowed_tools": ["call_model", "search_code", "read_file", "inspect_workspace"], "temperature": 0.3},
                    {"name": "patcher", "description": "Reads source files, produces a unified diff patch, and self-verifies — all in one turn.", "instructions": "You are fixing a real bug in a Python codebase. You work in a single turn: read, patch, verify, fix, output.\n\nWORKFLOW (do all of this in one response, using tools):\n1. read_file('artifacts/test_failures.txt') — see what tests fail\n2. Read the source files the locator found. Also read the problem_statement from the case input.\n3. Understand the root cause — what code path leads to the test failure?\n4. Write the patch using write_patch\n5. Verify the patch applies using apply_patch_dry_run\n6. If it fails: read the error, fix the patch (fix line numbers, fix context), re-verify\n7. Repeat step 6 up to 2 times until the patch applies cleanly\n8. Output ONLY the final raw unified diff — no markdown fences, no explanation\n\nYour patch must:\n- Fix the bug so the failing tests pass\n- Change as few lines as possible (ideally 1-5 lines)\n- Preserve existing code style and conventions\n- Use exact line numbers from the actual files you read\n\n=== PATCH FORMAT EXAMPLES ===\n\nExample 1 — fixing a missing import:\n--- a/module/file.py\n+++ b/module/file.py\n@@ -1,5 +1,6 @@\n import os\n+from collections import OrderedDict\n import sys\n\nExample 2 — fixing wrong comparison:\n--- a/src/utils.py\n+++ b/src/utils.py\n@@ -15,7 +15,7 @@\n def validate(value):\n-    if value > 0:\n+    if value >= 0:\n         return True\n\n=== END EXAMPLES ===\n\nOutput ONLY the raw unified diff. No markdown fences, no explanation, no wrapping text.\nStart with '--- a/filepath' or 'diff --git a/filepath'.", "allowed_tools": ["call_model", "read_file", "write_patch", "apply_patch_dry_run"], "temperature": 0.7},
                ...
                "workflow": [
                    {"id": "locate", "role": "locator", "action": "FIRST read artifacts/test_failures.txt and artifacts/test_file_hints.txt. THEN search the codebase for files mentioned in the tracebacks. Read the key files. Report file paths, line numbers, code snippets, and your hypothesis for the fix.", "input_from": [], "output_key": "code_locations"},
                    {"id": "self_verifying_patch", "role": "patcher", "action": "Read test_failures.txt, the locator's report, and the problem_statement. Understand the bug. Write a patch, verify it with apply_patch_dry_run, fix if needed, and output ONLY the final corrected unified diff.", "input_from": ["code_locations"], "output_key": "final_output"},
