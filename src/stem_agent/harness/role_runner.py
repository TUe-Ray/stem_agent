...
class RoleRunner:
    ...
    def run(
        ...
        temperature = role.temperature if role.temperature is not None else 0.0
        response = self.model_client.call(prompt, tools=tools, temperature=temperature, role=role.name)
        return str(response)
