        ...
        return Genome.model_validate(
            {
                ...
                "task_diagnosis": {},
                "roles": [
                    {"name": "locator", "description": "Searches source tree for relevant SOURCE code locations using test failures as guide.", "instructions": "You have access to the full repo source code in the workspace.\n\nCRITICAL RULE: You are looking for PRODUCTION/SOURCE code, NOT test code.\nDO NOT search in test/, tests/, testing/, or any directory with \"test\" in its name.\nIgnore test files entirely — only find the buggy production code.\n\nFIRST: read_file('artifacts/test_failures.txt') — see which tests fail and error messages.\nSECOND: read_file('artifacts/test_file_hints.txt') — see likely SOURCE modules (not test files).\n\nTHEN: use search_code to find the PRODUCTION source files, functions, and classes from:\n- The error tracebacks (function names, class names, module paths)\n- The problem statement (what's broken)\n- The SOURCE directories from hints file (e.g. sympy/integrals/ NOT sympy/integrals/tests/)\n\nUse read_file ONLY on source/production files. Never read test files.\n\nReport: (1) failing test names and expectations, (2) exact SOURCE file paths and line numbers, (3) buggy code snippets, (4) your hypothesis for the fix.", "allowed_tools": ["call_model", "search_code", "read_file", "inspect_workspace"], "temperature": 0.3},
                    {"name": "patcher", "description": "Reads test code + source files, produces a patch, self-verifies — all in one turn.", "instructions": "You are fixing a real bug in a Python codebase. You work in a single turn.\n\nWORKFLOW (do all in one response, using tools):\n1. read_file('artifacts/test_failures.txt') — see what tests fail and the errors\n2. read_file('artifacts/test_file_hints.txt') — see test file paths\n3. **CRITICAL: READ THE TEST FILES** — use read_file on the test file paths. The test code tells you EXACTLY what behavior is expected. Study the assertions — what inputs, what expected outputs?\n4. Read the source files the locator found\n5. Read the problem_statement from the case input\n6. Match: test expectations vs. actual code behavior → find the gap\n7. Write the patch using write_patch (patch the SOURCE, never the test)\n8. Verify with apply_patch_dry_run\n9. If it fails: read the error, fix line numbers/context, re-verify (up to 2 retries)\n10. Output ONLY the final raw unified diff\n\nYour patch must:\n- Fix the source code so tests pass\n- Change as few lines as possible (ideally 1-5 lines)\n- Preserve existing code style\n\n=== PATCH FORMAT ===\n--- a/sympy/module/file.py\n+++ b/sympy/module/file.py\n@@ -line,count +line,count @@ context line\n unchanged line\n-changed line\n+fixed line\n unchanged line\n\nOutput ONLY the raw diff. No markdown fences.", "allowed_tools": ["call_model", "read_file", "write_patch", "apply_patch_dry_run"], "temperature": 0.7},
                ...
                "workflow": [
                    {"id": "locate", "role": "locator", "action": "FIRST read artifacts/test_failures.txt and artifacts/test_file_hints.txt. THEN search ONLY source/production directories (NOT test dirs). Find the buggy production code. Read the source files. Report source file paths, line numbers, code snippets, and your fix hypothesis.", "input_from": [], "output_key": "code_locations"},
                    {"id": "self_verifying_patch", "role": "patcher", "action": "Read test_failures.txt AND the TEST FILES (from test_file_hints.txt). Read the locator's source report and the problem_statement. Study what the tests expect vs what the code does. Write a patch, self-verify, fix if needed. Output ONLY the final corrected unified diff.", "input_from": ["code_locations"], "output_key": "final_output"},
