        ...
        return Genome.model_validate(
            {
                ...
                "workflow": [
                    {"id": "locate", "role": "locator", "action": "FIRST read artifacts/test_failures.txt to see failing tests and error messages. THEN search the codebase for files mentioned in the tracebacks. Read the key files. Report file paths, line numbers, code snippets, and your hypothesis for the fix.", "input_from": [], "output_key": "code_locations"},
                    {"id": "read_and_patch", "role": "patcher", "action": "Read artifacts/test_failures.txt and the files the locator found. Understand the bug. Produce a minimal unified diff that makes the tests pass. Output ONLY the raw unified diff.", "input_from": ["code_locations"], "output_key": "final_output"},
                    ...
                    {"id": "revise_patch", "role": "patcher", "action": "The verification found issues with the patch. Read the verification result and test_failures.txt carefully. If it says 'Patch applies cleanly', output the original patch unchanged. If it reports errors, fix the patch — correct the file paths, line numbers, or context to match the actual files. Remember what the tests expect. Output ONLY the corrected unified diff.", "input_from": ["final_output", "verification_result", "code_locations"], "output_key": "final_output"},
